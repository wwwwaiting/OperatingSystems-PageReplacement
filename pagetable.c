...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// note only dirty page need to be swaped, need to be checked
		if (victim_page->frame & PG_DIRTY) {   // Write victim page to swap
			victim_page->frame |= PG_ONSWAP;
			evict_dirty_count++;

			// Update swap offset
			int swap_off = swap_pageout(frame, victim_page->swap_off);
			if (swap_off ==	INVALID_SWAP ) {
				fprintf(stderr, "allocate_fram: swap offset is invalid");
				return -1;
			}
			victim_page->swap_off = swap_off;

			// Set dirty bit to 0
			victim_page->frame &= ~(PG_DIRTY);
		} else {                               // Victim page is clean
			evict_clean_count++;
		}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *pde = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
	...
	if (p->frame & PG_VALID) { // valid
		...
		// allocate frame for p and update related variable
		...
		if (p->frame & PG_ONSWAP) {     // on swap
			...
			if (success != 0) { // error checking for swap
				fprintf(stderr, "find_physpgae: failed to read from swap file.");
				exit(1);
			}
			//update frame of p
